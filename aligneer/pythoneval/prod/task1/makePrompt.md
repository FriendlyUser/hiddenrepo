...
---
I'm trying to perform some encrypted computations using homomorphic encryption with the TenSeal library. I've got two vectors, and I need to calculate their encrypted dot product without decrypting them. But something's not working—I'm not getting the expected result.
...
# Decrypt the result
decrypted_result = enc_result.decrypt()[0]  # Dot product results in a single value

print("Encrypted dot product result:", decrypted_result)
```

When I run this code, I get an error that doesn't make much sense. Maybe I'm missing something with how TenSeal handles encrypted operations? Any idea why this isn't working as intended?
---

Assume the error message is

---
Traceback (most recent call last):
  File "task1.py", line 22, in <module>
    enc_vector_a = encrypt_vector(vector_a, context)
                   ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "task1.py", line 4, in encrypt_vector
    return ts.ckks_vector(context, vector)
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "\Python\Python312\Lib\site-packages\tenseal\__init__.py", line 102, in ckks_vector
    return CKKSVector(*args, **kwargs)
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "\Python\Python312\Lib\site-packages\tenseal\tensors\ckksvector.py", line 42, in __init__
